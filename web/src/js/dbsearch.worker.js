import quickselect from 'qselect';
...
self.onmessage = e => {
  ...
  e.data.q = dbutil.canonize(e.data.q).trim();
...
function fetchLectureOnDB() {
  ...
  var trigramPromises = dbutil.trigramsFromString(search.q,true).map(trigram => new Promise((resolve, reject) =>  {

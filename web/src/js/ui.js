...
UI.prototype.calcPositionForTime = function(schedule, timeBegin, timeEnd) {
  var positionBegin = (1 / (timeEnd - timeBegin)) * ((schedule.timeBegin.getHours()*60 + schedule.timeBegin.getMinutes()) / 60 - timeBegin);
  var positionEnd = 1 - (1 / (timeEnd - timeBegin)) * ((schedule.timeEnd.getHours()*60 + schedule.timeEnd.getMinutes()) / 60 - timeBegin);
...
UI.prototype.setCombinationPage = function(pageIndex) {
  ...
  var fragment = document.createDocumentFragment();
  for(var i = this.combinationTrackStart; i < this.combinationTrackEnd; i++) {
...
UI.prototype.makeTimeTable = function() {
  ...
  for(var i = timeBegin; i <= timeEnd; i++){
    var tick = i%24;
    this.timeColumn.appendChild(createHtmlElementTree({tag: 'div', 'class':'hour', 'innerHTML': tick}));
    var j = i-timeBegin;

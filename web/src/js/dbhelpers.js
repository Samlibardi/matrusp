...
global.dbutil = {
  stopwords_ptBR: [
    ...
    trigramsFromString: function(str,asAcronym) {
      ...
      if (words.length > 1 && !isNaN(words[words.length - 1]))
        words[words.length - 1] = dbutil.romanize(parseInt(words[words.length - 1]));

      words = words.filter(function(word,i) { 
          return word !== "" && (i == 0 || dbutil.stopwords_ptBR.indexOf(word) === -1);
      }.bind(this));
...
global.matruspDB = new Dexie("MatruspDB");

...
async def main():
	...
	codigos_unidades = {key:value for (key, value) in map(lambda x: (x.string, re.search("publico/lista/disciplina/(\d+)", x.get('href')).group(1)), links_unidades)}
	...
	if not args.unidades:
		materias = await iterar_unidades(codigos_unidades.values())
	else:
		materias = await iterar_unidades(args.unidades)
...
async def iterar_unidade(codigo):
	...
	response = await session.get('https://uspdigital.usp.br/janus/AreaListaPublico?tipo=T&codcpg=' + codigo, timeout = 120)
	...
	for area in codigos_areas.values():
		logger.debug(f' -		Obtendo as materias da area de concentração {area}')
		response = await session.get('https://uspdigital.usp.br/janus/TurmaLista?codare=' + area, timeout = 120)
		assert response.status == 200
		soup = BeautifulSoup(await response.text(), "html5lib")
		links_materias = links_materias + soup.find_all('a', href=re.compile('publico/turma'))
...
def parsear_turma(tabela):
	info = {}
	turma = {}
	professores = vagas = horario = None
	...
	info['nome'] = ''.join(trs[1].stripped_strings)
	...
	for tr in trs:
		if tr.find(class_="intl_numeroVagas"):
			...
			turma['professores'] = [''.join(x.stripped_strings) for x in tr.find_all('tr')][1:]
...
def parsear_vagas(tabela):
	...
	vagas = {
		'regular': int(tds[0].string if (tds[0].string != '-') else tds[2].string), 
		'especial': int(tds[1].string if (tds[1].string != '-') else 0)
		}
